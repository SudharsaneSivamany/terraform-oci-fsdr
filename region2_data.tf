...
data "oci_disaster_recovery_dr_protection_groups" "region2_dr_protection_groups" {
...
locals {
  region2_get_protection_group           = try(data.oci_disaster_recovery_dr_protection_groups.region2_dr_protection_groups.dr_protection_group_collection[0].items[0].id, null)
  region2_get_members                    = local.region2_get_protection_group != null ? data.oci_disaster_recovery_dr_protection_group.region2_dr_protection_group[0].members : []
  region2_member_id                      = [for entry in local.region2_get_members : entry.member_id]
  region2_modification_members           = { for entry in var.region2_config["add_members"] : entry.member_id => entry }
  region2_member_id_modification_members = [for entry in local.region2_modification_members : entry.member_id]
  region2_member_index_modification      = toset(setintersection(local.region2_member_id, local.region2_member_id_modification_members))
  region2_members_after_structure = [
    for entry in local.region2_get_members : {
  ...
  } if !contains(var.region2_config["remove_members"], entry.member_id)]

  region2_modified_members = [for entry in local.region2_members_after_structure : try(local.region2_modification_members[entry.member_id], entry)]
  region2_append_members   = [for key, value in local.region2_modification_members : value if !contains(local.region2_member_index_modification, key)]
  region2_altered_members  = concat(local.region2_modified_members, local.region2_append_members)

}


data "oci_disaster_recovery_dr_protection_group" "region2_dr_protection_group" {
  count                  = local.region2_get_protection_group == null ? 0 : 1
  provider               = oci.region2
  dr_protection_group_id = data.oci_disaster_recovery_dr_protection_groups.region2_dr_protection_groups.dr_protection_group_collection[0].items[0].id
}
